...
const Spinner: FC<{ color: ColorVariantEnum, size?: SizeEnum } & HTMLAttributes<HTMLOrSVGElement>> = ({color = ColorVariantEnum.Primary, size, ...props}) => {
  const className = useMemo(() => {
    ...
    switch (size) {
      ...
      case SizeEnum.Small:
        _className.push('w-3 h-3');
        break;
      case SizeEnum.Large:
        _className.push('w-5 h-5');
        break;
      default:
        _className.push('w-4 h-4');
        break;
    }
  ...
  return (
    <svg
      className={classNames(props.className, className, 'text-gray-200 animate-spin dark:text-gray-600')}
      viewBox="0 0 100 101"
      fill="none"
    ...
    >
      ...
      <path
        ...
        className={classNames(`fill-${getColorClassName({color})}`)}
      />
    </svg>
  )
}
