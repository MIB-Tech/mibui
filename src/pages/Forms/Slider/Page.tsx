	...
	return (
		<div className='flex flex-col gap-12'>
			...
			<Preview title='Marks with label'>
				<Slider marks={[0, 25, 50, 75, 100].map(value => ({value, label: `${value}C°`}))}/>
			</Preview>
			<Preview title='Custom label' className='gap-y-10'>
				{DISPLAYS.map(display => (
					<div key={display} className='text-center'>
						<Slider tooltipDisplay={display}/>
						<div className='capitalize'>{display}</div>
					</div>
				))}
			</Preview>
			<Preview title='Range slider'>
				<Slider defaultValue={[20, 37]}/>
			</Preview>
			<Preview title='Vertical' className="justify-center">
