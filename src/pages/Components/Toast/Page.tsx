...
import {ColorVariantEnum} from "../../../Components/Button/Button.types.tsx";
...
const Page: React.FC = () => {
	...
	return (
		...
		<Preview title="Default" className="flex items-center justify-center gap-2">
			<Button
				onClick={() => notify({title: `here's your toast`})}
			>
				Make me a toast
			</Button>
		</Preview>
		<Preview title="Dismissible" className="flex items-center justify-center gap-2">
			<Button
				onClick={() => notify({
					title: `Dismissible`, dismissible: true
				})}
			>
				Dismissible
			</Button>
		...
		<Preview title="With icon" className="flex items-center justify-center gap-2">
			<Button
				onClick={() => notify({
					title: `With icon`, icon: <CheckIcon className='h-5 w-5 mt-1'/>
				})}
			>
				With icon
			</Button>
		...
		<Preview title="Duration" className="flex items-center justify-center gap-2">
			{[1, 3, 10].map(durationPerSec => {
				...
				return (<Button
					key={durationPerSec}
					onClick={() => notify({
						title, duration: durationPerSec * 1000
					})}>
					{title}
				</Button>)
			})}
		...
		<Preview title='Colors' className="grid gap-4 grid-cols-2">
			{Object.values(ColorVariantEnum).map(color => (<Toast
				title={`${color} Toast`}
				color={color}
